...
import(src).then(module => {
    ...
    .promper-enhancing {
        position: relative;
        background: linear-gradient(90deg, rgb(240 244 249), rgb(211 227 253), rgb(145 203 255));
    ...
    .promper-enhancing::after {
        ...
        font-weight: 500;
        color: #ffffff;
        background-color: rgb(95 143 253);
        opacity: 0;
...
`;
    ...
    function extractLastMessages(count = 5) {
        const allElements = Array.from(document.querySelectorAll("user-query-content > * .query-text"));
        const messages = [];
    ...
    function initializeTextboxListerener() {
        const textarea = document.querySelector("rich-textarea > div");
        if (!textarea) return;
